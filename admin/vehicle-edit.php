...
$vehiclee_id=$_GET['id'];
$vehicle_edit_query="select * from tbl_vehicle where id='".$vehiclee_id."'";
$vehicle_edit_data=$conn->query($vehicle_edit_query);
if ($vehicle_edit_data->num_rows ==0) 
    {
         header("location:vehicle-list.php");
    }

$vehicle_edit_arr = $vehicle_edit_data->fetch_assoc();

if(isset($_POST) && !empty($_POST))
{
	$name=$_POST['vehicle-name'];
    $vehicle_id_in=$_POST['vehicle_id'];
    $reg=$_POST['reg'];
    ...
    $userquery="update tbl_vehicle set name='".$name."',reg_no='".$reg."',vehicle_type_id='".$type_id."',make_id='".$make_id."',model_id='".$model_id."',color='".$color."',mfl_condition='".$condition."',transport_id='".$transport_id."',updated_on='".$today_date."' where id='".$vehicle_id_in."'";

    $result = $conn->query($userquery);

    if(isset($_FILES) && !empty($_FILES)){
        $upload_pass="../uploads/transport/";
        $new_file="usr_".date("ymdhis").$vehicle_id_in.basename($_FILES['vehicle_image']['name']);
        $destination=$upload_pass.$new_file;
        if(move_uploaded_file($_FILES['vehicle_image']['tmp_name'],$destination)){
            $vehicleupdate="update tbl_vehicle set photo='".$new_file."' where id='".$vehicle_id_in."'";
...
<head>
    ...
    <title><?php echo ADMIN_VEHICLE_UPDATE_TITLE; ?></title>
                    ...
                    <div class="col-lg-12 col-xlg-12 col-md-12">
                        <div class="card">
                            <div class="card-body">
                                ...
                                <input type="hidden" name="vehicle_id" value="<?php echo $vehicle_edit_arr['id']?>"/>
                                    <div class="form-group mb-4">
                                        <label class="col-md-12 p-0">Vehicle Name</label>
                                        <div class="col-md-12 border-bottom p-0">
                                            <input type="text" name="vehicle-name" placeholder="Enter Vehicle"
                                                class="form-control p-0 border-0" value="<?php echo $vehicle_edit_arr['name']?>" required>
                                        </div>
                                    </div>
                                    ...
                                    <div class="form-group mb-4">
                                        ...
                                        <div class="col-md-12 border-bottom p-0">
                                            <input type="text" name="reg" placeholder="Enter Reg No"
                                                class="form-control p-0 border-0" value="<?php echo $vehicle_edit_arr['reg_no']?>" required>
                                        </div>
                                    </div>
                                    ...
                                    <div class="form-group mb-4">
                                        ...
                                        <div class="col-md-12 border-bottom p-0">
                                            <input type="text" name="color" placeholder="Enter Color"
                                                class="form-control p-0 border-0" value="<?php echo $vehicle_edit_arr['color']?>" required>
                                        </div>
                                    </div>
